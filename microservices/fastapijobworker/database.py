...
async def init_pool() -> None:
    ...
    _pool = await aiomysql.create_pool(
        ...
        # Pool sizing: small pool is fine — worker is single-threaded asyncio
        minsize=2,
        maxsize=10,
        # Return DictRow by default for ergonomic access
...
async def claim_job(pool: aiomysql.Pool) -> dict | None:
    ...
    This is the core of the MySQL-based job queue.
    SKIP LOCKED means concurrent workers never block each other — each sees
    only unlocked rows. If no unlocked queued row exists, returns None.
    ...
    async with pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            try:
                ...
                # Claim the oldest queued job that hasn't exceeded max attempts
                await cur.execute(
                    """
